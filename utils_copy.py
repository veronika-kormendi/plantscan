...
import pandas as pd
# spell = SpellChecker(distance=3) # load default word frequency list
spell = SpellChecker() # load default word frequency list
extra_words = ["flavouring", "sucralose", "caramelised", "colours", "sorbate", "flavour", "guar", "thermophilus",
               "bulgaricus", "lecithins", "folic", "fibre", "sucralose", "stabiliser", "stabilisers", "curcumin",
               "xanthan", "sundried", "crouton", "croutons", "colour", "humectants", "acerola", "pasteurised", "coagulans", "flavourings", "sugar"]
for w in extra_words:
    spell.word_frequency.add(w) #add extra words

word = "sugar"
word2 = "star"

is_known1 = word in spell.word_frequency.dictionary
is_known2 = word2 in spell.word_frequency.dictionary

print(f"{word} known: {is_known1}")
print(f"{word2} known: {is_known2}")

freq1 = spell.word_usage_frequency(word) if is_known1 else 0.0
freq2 = spell.word_usage_frequency(word2) if is_known2 else 0.0

print(f"frequency of {word}: {freq1}")
print(f"frequency of {word2}: {freq2}")
...
#                     f.write(w + "\n")
##------------working version -each word in new line-----------------
# def postprocess_text(input_folder_path, output_folder_path):
#     """function to postprocess multiple text files
#     :param input_folder_path: path to folder containing text files
...
#             if w in misspelt: # if word is in misspelt
#                 candidates = spell.candidates(w) #possible corrections
#                 if candidates: #if there are any suggestions
#                     for cand in candidates: # go through possible corrections
#                         freq = spell.word_usage_frequency(cand) #candidate's freq
#                         if freq > best_freq:
#                             best_freq = freq
#                             best = cand
#                     # best = max(candidates, key=lambda c: spell.word_usage_frequency(c))
#                     if Levenshtein.distance(w, best) <= 1:
#                         corrected_text.append(best)
#                         continue
#             corrected_text.append(w)
#         base = filename.replace("_cropped_cleaned_ocr.txt", "")
#         new_filename = f"{base}_postprocessed.txt"
