...
# batch process ocr
#
def perform_ocr_multiple(img_path, ocr_output_folder_path="ocr_txt_folder"): #folder
    ...
    for json_file in os.listdir("output"): # extract text from json files
        ...
        txt_path = os.path.join(ocr_output_folder_path, f"{base}.txt") # ocr_txt_folder
        with open(json_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        text_lines = data.get("rec_texts", [])
        with open(txt_path, "w", encoding="utf-8") as f:
...
annotation_folder = 'C:\\Users\\veron\\PycharmProjects\\plantscan\\annotation'
# test_gt_filepath = 'C:\\Users\\veron\\PycharmProjects\\plantscan\\annotation\\IMG_7762_annotation_pb.txt' # temporarily I use this path for testing
def save_crop_and_start_ocr():
    ...
    if cropped_img_path is None:
        return
    # 2. extract text
    ocr_extracted = perform_ocr_multiple(cropped_folder) # for multiple images
...
################
    ...
    for root, dirs, files in os.walk(ocr_extracted): # go through the content of the folder and
        for file in files: # for every file
            ...
            display_extracted = load_words(file)
            print(f"ocr text before cleaning: {display_extracted}")
            cleaned_extracted = count_word_and_char(display_extracted)
    ...
    # if extracted_txt is not None:
        ...
        # extracted_text_before_cleaning = load_text(extracted_txt) # getting the extracted text
        # print(f"extracted text before cleaning: {extracted_text_before_cleaning}") # display extracted text before cleaning
        # print(f"cleaned_extracted text's path: {cleaned_extracted_test}") #display filepath of currently processed file
        # # load_cleaned_extracted = load_words(cleaned_extracted_test)
        # load_cleaned_extracted = load_text(cleaned_extracted_test) # getting the cleaned, extracted text
        # print(f"loaded cleaned_extracted text: {load_cleaned_extracted}") # display extracted text after cleaning

    # clean all annotated text files in the annotated folder
    #     for root, dirs, files in os.walk(annotation_folder): # go through all content in annotation_folder
    #         for file in files: # for every file
    #             annotated_filepath = os.path.join(root, file) # create annotated filepath
    #             print(annotated_filepath) # print annotated filepath
    #             not_cleaned_gt_text = load_text(annotated_filepath)
    #             print(f"not_cleaned_gt_text: {not_cleaned_gt_text}")
    #             cleaned_gt_text = count_word_and_char(annotated_filepath, file_type="gt")
    #             load_cleaned_gt_text = load_text(cleaned_gt_text)
    #             print(f"load_cleaned_gt_text: {load_cleaned_gt_text}")
...
# def batch_process_text(batch_input, batch_output):
#     processed_folder
...
cleaned_folder_path = 'C:\\Users\\veron\\PycharmProjects\\plantscan\\cleaned_folder_test'
